...
try:
    from .bootstrap import activate_runtime_packages, ensure_all_dependencies, ensure_core_dependencies, ensure_optional_dependency_group
except ImportError:
    _bootstrap_path = Path(__file__).with_name("bootstrap.py")
    _bootstrap_spec = importlib.util.spec_from_file_location("subburn_bootstrap_local", _bootstrap_path)
    _bootstrap_mod = importlib.util.module_from_spec(_bootstrap_spec)
    sys.modules[_bootstrap_spec.name] = _bootstrap_mod
    _bootstrap_spec.loader.exec_module(_bootstrap_mod)
    activate_runtime_packages = _bootstrap_mod.activate_runtime_packages
    ensure_all_dependencies = _bootstrap_mod.ensure_all_dependencies
    ensure_core_dependencies = _bootstrap_mod.ensure_core_dependencies
    ensure_optional_dependency_group = _bootstrap_mod.ensure_optional_dependency_group
...
def ensure_fonttools_runtime():
    """Load FontTools, preparing the private SubBurn runtime if needed."""
    try:
        ...
        raise RuntimeError("Font rendering support could not be prepared: " + str(exc)) from exc
...
def ensure_grapheme_pattern():
    ...
    with _GRAPHEME_PATTERN_LOCK:
        ...
        try:
            ...
            raise RuntimeError("Unicode text support could not be prepared: " + str(exc)) from exc
        try:
...
def ensure_faster_whisper_runtime(allow_install=False):
    """Load the local transcription runtime, preparing it when needed."""
    ...
    with _FASTER_WHISPER_LOCK:
        if _FASTER_WHISPER_RUNTIME is not None:
            return _FASTER_WHISPER_RUNTIME
        try:
            ensure_optional_dependency_group("transcription", required=True)
            ...
            raise RuntimeError("Local transcription support could not be prepared: " + str(exc)) from exc
...
def _huggingface_snapshot_download(**kwargs):
    try:
        ...
        raise RuntimeError("Transcription download support could not be prepared: " + str(exc)) from exc
    return fn(**kwargs)
...
def _huggingface_snapshot_download_cancellable(source, local_dir, cancel_event, *, poll_interval=0.20, _command=None):
    ...
    with tempfile.TemporaryFile(mode="w+b") as child_log:
        ...
        runtime_pydeps = str(activate_runtime_packages())
        child_env["PYTHONPATH"] = runtime_pydeps + (os.pathsep + child_env["PYTHONPATH"] if child_env.get("PYTHONPATH") else "")
...
def load_tkinterdnd(allow_install=False, retry_seconds=86400):
    """Load drag/drop support when the private runtime already has it."""
    ...
    try:
        ...
        DND_RUNTIME_ERROR = "Drag and drop is not ready: " + str(exc)
        return None
...
def main():
    threading.Thread(target=ensure_all_dependencies, kwargs={"required": False}, daemon=True).start()
    root = create_root()
    app = SubBurnApp(root)
